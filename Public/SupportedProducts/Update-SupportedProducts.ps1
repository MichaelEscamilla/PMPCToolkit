function Update-SupportedProducts {
    ...
    try {
        ...
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] Successfully downloaded the supported products XML file from [$SupportedProductsUrl]"
        $SupportedProductsXmlRaw.Content | Out-File -FilePath $SupportedProductsFileFullName -Encoding utf8 -Force
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] Successfully saved the supported products XML file to [$SupportedProductsFileFullName]"
    }
    catch {
