<#
    .SYNOPSIS
    Retrieves ConfigMgr Auto-Publish settings from Patch My PC Publisher backup files.

    .DESCRIPTION
    This function extracts and displays the ConfigMgr (Configuration Manager) Auto-Publish
    configuration settings from Patch My PC Publisher backup CAB files. It locates the restored
    backup folders and searches each Settings.xml file for ConfigMgr Auto-Publish application
    and update enrollment settings, including their enabled state and configured thresholds.
    ...
    Searches the current directory for backup files and displays the ConfigMgr Auto-Publish settings.
#>
function Get-PMPCBackupSettingsAutoPubConfigMgr {
    [CmdletBinding()]
    param(
    ...
    Search-BackupSettingsAutoPubConfigMgr -SettingsFileObject $BackupRestoredFoldersObject
}